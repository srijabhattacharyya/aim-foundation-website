...
export default function SoulCircleDonationForm() {
  ...
  async function onSubmit(values: z.infer<typeof donationSchema>) {
    ...
    try {
        const donationData = { ...values, cause: 'SoulCircle', createdAt: serverTimestamp(), dob: values.dob ? format(values.dob, 'yyyy-MM-dd') : null };
        await addDoc(collection(db, "donations"), donationData);
        toast({
        title: "Thank you for supporting SoulCircle!",
        description: "Your donation helps provide mental health support.",
        });
    ...
    <Card className="w-full border-0 shadow-none rounded-none">
        <CardContent className="p-6 md:p-8" onFocus={() => setShowRecaptcha(true)} onClick={() => setShowRecaptcha(true)}>
            ...
            <div className="text-center mb-8 pt-20">
                <h2 className="text-3xl font-bold font-headline">SUPPORT SOULCIRCLE</h2>
                <p className="text-muted-foreground">PROVIDE A SAFE SPACE</p>
            </div>
